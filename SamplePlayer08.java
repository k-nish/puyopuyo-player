...
public class SamplePlayer08 extends AbstractSamplePlayer {
		...
		for(int i = 0; i < field.getWidth(); i++){
			for(PuyoDirection dir:PuyoDirection.values()){
				...
				if(dir == PuyoDirection.DOWN){
					//二番目のぷよが下(正しくは上)にある場合は，topの二つ上がy座標
					//int x = i;
				...
				if(dir == PuyoDirection.DOWN){
					...
					//二番目のぷよが右にある場合
					int x = i + 1;
					int y = field.getTop(i);
					secondNeighbor = getNeighborPuyoNum(x, y, secondPuyo);
				}
				...
				action = new Action(dir, maxColumnNum);
			}
		}
		if(action != null){
	...
	private boolean isEnable(PuyoDirection dir, int i) {
		...
		if(dir == PuyoDirection.DOWN || dir == PuyoDirection.UP){
			if(field.getTop(i) == 11){
				return false;
			}
		}
		else if(dir == PuyoDirection.RIGHT){
			if(field.getTop(i) == 12 || i == 0) {
				return false;
			}
		}
		else if(dir == PuyoDirection.LEFT){
			if(field.getTop(i) == 12 || i == field.getWidth()) {
				return false;
			}
		}
		return true;
	...
	private int getNeighborPuyoNum(int x, int y, PuyoType puyoType) {
		...
		if(x<field.getWidth() && puyoType == field.getPuyoType(x+1,y)){
			count++;
		}
